...
describe('migration runner ordering', () => {
  it('awaits migrations sequentially and reports success only after 007 completes', () => {
    ...
    const userActivity = source.indexOf('await sql.unsafe(userActivity).simple()');
    ...
    expect(userActivity).toBeGreaterThan(adminLab);
    expect(success).toBeGreaterThan(userActivity);
  });
